...
const Donate = ({navigation}) =>{
  ...
  const sendBody = () => {
    ...
    axios.post(`http://${url}/donate`, body)
    .then((res) => {
      if (res.data != false){
  ...
  const refreshPlaces = () => {
    axios.get(`http://${url}/places`)
    .then((res) => {
      console.log(res.data);
      //setPlaces(res.data.map(function(item, index){ return {label : index, value : item.place}}));
      setPi(res.data.map((item, index) => {return <Picker.Item key={index} label={item.place} value={item.place}/>}));
    })
    .catch((err) => {
